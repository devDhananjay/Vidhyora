...
export const EMAIL_LOGO_SRC = "/brand/vidyora-monogram-clear.png";
...
export function wrapBrandEmail(options: {
  ...
  const appUrl = getPublicEmailOrigin();
  const year = new Date().getFullYear();
  const logoSrc = emailAssetUrl(EMAIL_LOGO_SRC);
  const heroSrc = options.heroImage ? emailAssetUrl(options.heroImage) : "";
  const heroAlt = escapeHtml(options.heroAlt || APP_NAME);
  const ctaUrl = options.cta?.url ? emailAssetUrl(options.cta.url) : appUrl;
  const heroLink = emailAssetUrl(options.heroHref || ctaUrl);

  const hero = heroSrc
    ? `<tr>
            <td style="padding:0;line-height:0;font-size:0;">
              <a href="${escapeHtml(heroLink)}" style="display:block;text-decoration:none;">
                <img src="${escapeHtml(heroSrc)}" alt="${heroAlt}" width="600" style="display:block;width:100%;max-width:600px;height:auto;border:0;outline:none;text-decoration:none;" />
              </a>
            </td>
          </tr>`
    : "";
  ...
  const cta = options.cta
    ? `<tr>
        <td align="center" style="padding:8px 0 28px;">
          <a href="${escapeHtml(ctaUrl)}" style="display:inline-block;background:${MAROON};color:#ffffff;text-decoration:none;font-family:Georgia,serif;font-size:14px;letter-spacing:0.12em;text-transform:uppercase;padding:14px 28px;border-radius:999px;">
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;width:100%;background:#ffffff;border:1px solid #efe8e2;border-radius:20px;overflow:hidden;">
          <tr>
            <td style="background:${MAROON};padding:28px 32px;text-align:center;">
              <img src="${escapeHtml(logoSrc)}" alt="${APP_NAME}" width="96" height="96" style="display:block;margin:0 auto;border:0;outline:none;text-decoration:none;" />

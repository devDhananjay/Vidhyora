...
export const commerceSettingsSchema = z.object({
  ...
  fastDeliveryEnabled: z.boolean().default(true),
  fastDeliveryFee: z.coerce.number().min(0).max(10_000).default(99),
  ...
  returnWindowDays: z.coerce.number().int().min(0).max(90).default(5),
  maxReturnsBeforeBlock: z.coerce.number().int().min(0).max(100).default(5),
